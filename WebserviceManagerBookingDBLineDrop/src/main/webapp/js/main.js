/* Goi den url, view, service, controller */
		...
		var bookingState = {
			...
			resolve : {
				bookings : function($q, BookingService) {
					...
					return deferred.promise; // tra ve ket qua c/v duoc thuc hien
				}
			}
		};
		...
		var bookborrowingState = {
			...
			resolve : {
				bookborrowings : function($q, BookBorrowingService){
					...
					BookBorrowingService.loadAllBookBorrowings().then(deffered.resolve, deffered.resolve);
